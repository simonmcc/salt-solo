...
parser.add_argument('--password-fields', help="What are the password fields required by your vpn site.", default="Password:password,Secondary password:password#2")
...
def find_by_id(base, id):
  try:
    if dict(base.items())['id'] == id:
      return base
  except KeyError, AttributeError:
    pass

  for child in base.getchildren():
    answer = find_by_id(child, id)
    if answer is not None:
      return answer
...
class JuniperNCPrompt:
  ...
  def get_passwords(self):
    passwords = {}
    for pass_set in self.args.password_fields.split(','):
      (label, sep, field) = pass_set.partition(':')
      if 'HPCS_SSO_PW_HELPER' in os.environ and label == 'Password':
 	# exec the helper to get the password
        print("Retrieving password via {0} from HPCS_SSO_PW_HELPER".format(os.environ.get('HPCS_SSO_PW_HELPER')))
 	passwords[field] = subprocess.check_output(os.environ.get('HPCS_SSO_PW_HELPER'), shell=True).rstrip()
      else:
	passwords[field] = getpass.getpass(label+":")
    return passwords

  def get_user(self):
    if 'HPCS_SSO_USERNAME' in os.environ:
      print("Using {0} from HPCS_SSO_USERNAME".format(os.environ.get('HPCS_SSO_USERNAME')))
      self.args.username = os.environ.get('HPCS_SSO_USERNAME')
    else:
